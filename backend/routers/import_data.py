...
async def import_advertising(
    ...
    try:
        ...
        for row in reader:
            ...
            ad_spend = _safe_decimal(get_col(row, "花费(USD)", "花费(CAD)", "花费(MX)", "花费(EUR)", "花费(GBP)", "花费(SEK)", "花费", "支出(USD)", "支出(CAD)", "支出(MX)", "支出(EUR)", "支出(GBP)", "支出(SEK)"))
...
def _process_advertising_sheet(db, country_obj, header, rows, time_id=None, store_id=None, import_year=None, import_month=None):
    ...
    col_spend = _find_col(header, "花费(usd)", "花费(cad)", "花费(mx)", "花费(eur)", "花费(gbp)", "花费(sek)", "花费", "支出(usd)", "支出(cad)", "支出(mx)", "支出(eur)", "支出(gbp)", "支出(sek)", "spend", "cost")
